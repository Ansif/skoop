...
jQuery("#range-slider").slider({
  ...
  slide: function( event, ui ) {
    ...
    if(ui.value > 50) {
        screenText = '50+';
        $('.pricing-details-wrapper, .pricing-offer-notify-block').slideUp();
        $('.pricing-chat-block').slideDown();
        $('.pricing-line-block-2').css({'position': 'relative', 'top': '0'});
    } else {
        screenText = ui.value;
        $('.pricing-details-wrapper, .pricing-offer-notify-block').slideDown();
        $('.pricing-chat-block').slideUp();
        $('.pricing-line-block-2').css({'position': 'absolute', 'top': '20.5%'})
    }
   jQuery(this).find('.pricing-button-holder-wrapper span').text(screenText);
